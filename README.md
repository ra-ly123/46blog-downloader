...
ここから zip をダウンロードして使うこともできます。

README では後続のライブラリインストール説明の都合上、  
コマンド（git clone）での手順をメインに書いています。

とは言っても、このツールは  
Python / requests / beautifulsoup4 しか使っていないので、  
これらがすでに入っている人は zip をダウンロードして  
[使い方](#-使い方) に飛んじゃってください。

※ git が入っていない場合も zip ダウンロードで大丈夫です。
...
これらのアプリ(以後CLIと呼称)でコマンドを実行できます。(使えればどれでもいいです)
...
### 1. main.py を実行  
ダブルクリックで OK、ツールのフォルダから`cmd`を開いて以下を実行しても使えますが当然ダブルクリックの方が楽です。  
...
メンバーの一覧が表示されるので、保存したいメンバーの番号を入力しEnter  
